...
- 按照移动参数标准修改BU状态机消息转发，connect消息BU测对数据进行再封装后发给Adapter，BU测填充Type Mac IP，HOP、RouteIndex等数据到消息数据中、close消息 BU测监听，有设备close则BU给Adapter发消息，状态机收到所有到Adapter 的消息对数据段填充Type和Mac字段到数据中。
- Adapter为OAM增加设置oam参数接口，和告警上报接口。
- EU 实现 设置OAM状态参数消息相关逻辑，实现升级失败告警消息上报相关处理逻辑。
- EU实现接收OAM发过来的重启消息处理相关逻辑。
- EU 实现重启日志记录，记录存储到flash上，实现动态控制日志最大上限，目前规划最大记录50条，超过50条删除前20条。
- EU 制作软件包中 增加 小罡提供的eeprom工具和脚本，实现自动编译合入。
- 
